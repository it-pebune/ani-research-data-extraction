...
class Gift(TableInDocument):
    ...
    def create_from_row(self, row):
        ...
        self.owner = row[1] if 1 < len(row) else None
        self.source = row[2] if 2 < len(row) else None
        self.service = row[3] if 3 < len(row) else None
        self.year_income = row[4] if 4 < len(row) else None
    ...
    def to_string(self):
        return self.person_type.to_string() + ' - ' + self.owner.to_string() + ' - ' + self.source.to_string() + ' - ' + \
